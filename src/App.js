...
const AuthenticatedApp = () => {
  ...
  }, [endpoint, filters, showToast, token, logout])
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      if (res.status === 401) {
        // Don't show toast here as fetchTasks will handle it
        return
      }
  ...
  const addTask = useCallback(
    ...
    [endpoint, fetchStats, showToast, token, logout],
  )
  ...
  const sidebarComponent = useMemo(
    ...
    [activeView, handleViewChange, sidebarOpen, stats, logout],
  )
...
function App() {
  ...
  return (
    <ThemeProvider>
      <AuthProvider>
        {isAuthenticated ? <AuthenticatedApp /> : <AuthPage onAuthSuccess={() => setIsAuthenticated(true)} />}
      </AuthProvider>
    </ThemeProvider>
  )
}
